...
export function SortingBins() {
              ...
              <div className="w-full h-52 bg-gradient-to-br from-orange-50 to-red-50 border border-orange-200 rounded-2xl relative overflow-visible shadow-sm flex items-center justify-center">
                ...
                <div className="relative -mt-8">
                  ...
                  {[...Array(8)].map((_, i) => (
                    <motion.div
                      key={i}
                      className="absolute w-40 sm:w-48 h-12 bg-white border border-orange-200/50 rounded-lg shadow-sm transform -translate-x-1/2 left-1/2"
              ...
              <div className="w-full h-52 bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 rounded-2xl relative overflow-visible shadow-sm flex items-center justify-center">
                ...
                <div className="relative -mt-8">
                  ...
                  {[...Array(6)].map((_, i) => (
                    <motion.div
                      key={i}
                      className="absolute w-40 sm:w-48 h-12 bg-white border border-blue-200/50 rounded-lg shadow-sm transform -translate-x-1/2 left-1/2"
                      initial={{ y: 20, opacity: 0 }}
                      animate={{ 
  ...
  // Get the actual position of the sorting bins on the page
  const sortingBinsElement = document.querySelector('.grid.grid-cols-1.lg\\:grid-cols-2');
  const binElements = sortingBinsElement?.querySelectorAll('.max-w-sm');
  
  let leftBinX, rightBinX, binY;
  
  if (binElements && binElements.length >= 2) {
    // Get actual bin positions relative to the viewport
    const leftBinRect = binElements[0].getBoundingClientRect();
    const rightBinRect = binElements[1].getBoundingClientRect();
    
    // Calculate center of each bin
    leftBinX = leftBinRect.left + leftBinRect.width / 2;
    rightBinX = rightBinRect.left + rightBinRect.width / 2;
    binY = leftBinRect.top + leftBinRect.height / 2;
  } else {
    // Fallback to original calculation if elements aren't found
    const viewportWidth = window.innerWidth;
    const viewportHeight = window.innerHeight;
    leftBinX = viewportWidth < 1024 ? viewportWidth / 2 - 100 : viewportWidth * 0.3;
    rightBinX = viewportWidth < 1024 ? viewportWidth / 2 + 100 : viewportWidth * 0.7;
    binY = viewportHeight * 0.78;
  }
